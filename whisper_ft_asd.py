...
# === 路径设置 ===
SEGMENTS_TRAIN_DIR = "/scratch/s5910587/data_asd/segs_train"
GT_TRAIN_DIR = "/scratch/s5910587/data_asd/GT_train"
...
class LoggingCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        try:
            ...
            plt.figure(figsize=(10, 6), dpi=150)  # 更高分辨率
            plt.title("Training Metrics Over Steps", fontsize=14)
            ...
            if 'train_loss' in df.columns and df['train_loss'].notnull().any():
                plt.plot(df['step'], df['train_loss'], label="Train Loss", linewidth=2)
            if 'eval_loss' in df.columns and df['eval_loss'].notnull().any():
                plt.plot(df['step'], df['eval_loss'], label="Eval Loss", linewidth=2)
            if 'eval_wer' in df.columns and df['eval_wer'].notnull().any():
                plt.plot(df['step'], df['eval_wer'], label="Eval WER", linewidth=2)

            # === 自动设置 y 轴范围使图不空旷 ===
            max_y = df[['train_loss', 'eval_loss', 'eval_wer']].max(skipna=True).max()
            plt.ylim(0, max_y * 1.2)

            plt.legend()
            plt.tight_layout()
            plt.savefig("/scratch/s5910587/whisper_ft/asd_only_70/train_log.png")
            plt.close()
...
training_args = Seq2SeqTrainingArguments(
    output_dir="/scratch/s5910587/whisper_ft/asd_only_70/model",
...
trainer = Seq2SeqTrainer(
    ...
    callbacks=[LoggingCallback("/scratch/s5910587/whisper_ft/asd_only_70/train_log.tsv")], # 只保留日志回调
)
...
model.save_pretrained("/scratch/s5910587/whisper_ft/asd_only_70/final_model")
processor.save_pretrained("/scratch/s5910587/whisper_ft/asd_only_70/final_processor")

# === 保存最终训练结果 ===
final_metrics = trainer.state.log_history[-1]
import json
with open("/scratch/s5910587/whisper_ft/asd_only_70/final_metrics.json", "w", encoding="utf-8") as f:
