...
# === 路径设置 ===
SEGMENTS_TRAIN_DIR = "/scratch/s5910587/data_td_adhd_asd/segments"
GT_TRAIN_DIR = "/scratch/s5910587/data_td_adhd_asd/GT"
...
tokenizer.set_prefix_tokens(language="nl", task="transcribe")


# === 模型加载与 LoRA 支持 ===
model = WhisperForConditionalGeneration.from_pretrained("openai/whisper-medium", torch_dtype=torch.float32)
...
def prepare_dataset(batch):
    try:
        ...
        print(f"[ERROR] Failed to process sample: {batch.get('path', 'UNKNOWN')} | Error: {e}")
        return None
...
print(f"Train samples after map: {len(dataset['train'])}")
print(f"Validation samples after map: {len(dataset['validation'])}")
...
class LoggingCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        try:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig("/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/train_log.png")
            plt.close()
        except Exception as e:
...
training_args = Seq2SeqTrainingArguments(
    output_dir="/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/model",

    per_device_train_batch_size=4,
    gradient_accumulation_steps=4,
    learning_rate=3e-6,  # 降低一点学习率，更稳定
...
class Seq2SeqTrainer(HFSeq2SeqTrainer):
    def prediction_step(self, model, inputs, prediction_loss_only=False, ignore_keys=None):
        if self.args.predict_with_generate and not prediction_loss_only:
            ...
            gen_kwargs["forced_decoder_ids"] = processor.get_decoder_prompt_ids(language="nl", task="transcribe")
...
trainer = Seq2SeqTrainer(
    ...
    callbacks=[LoggingCallback("/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/train_log.tsv")], # 只保留日志回调
)
...
model.save_pretrained("/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/final_model")
processor.save_pretrained("/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/final_processor")

# === 保存最终训练结果 ===
final_metrics = trainer.state.log_history[-1]
import json
with open("/scratch/s5910587/whisper_ft/td_adhd_asd_lora_8000/final_metrics.json", "w", encoding="utf-8") as f:
